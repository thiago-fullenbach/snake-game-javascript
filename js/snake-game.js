window.onload = () => {
    ...
    const TopScoreCounter = document.getElementById('info-top-score')

    lifeCounter.innerHTML = lives
    scoreCounter.innerHTML = tail + 1
    
    if(!localStorage.TopScore)
        localStorage.TopScore = tail + 1
    TopScoreCounter.innerHTML = localStorage.TopScore
    ...
    function updateScore() {
        const novoComprimento = tail + 1
        scoreCounter.innerHTML = novoComprimento
        if(novoComprimento > localStorage.TopScore) {
            localStorage.TopScore = novoComprimento
            TopScoreCounter.innerHTML = novoComprimento
        }
    }
    ...
    function checkCollision() {
        trail.forEach(part => {
            ...
            if(part.x == px && part.y == py) { // checks collision with trail parts
                lives--
                resetGame()
            }
        })
    ...
    function snakeGame() { // Main function
        px += vx
        py += vy

        if(px > qp - 1) // Draw snake on the other end of the screen
            px = 0
        if(px < 0)
            px = qp - 1
        if(py > qp - 1)
